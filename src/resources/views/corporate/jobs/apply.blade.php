    ...
    <header class="header" role="banner">
        <div class="header-content">
            ...
            <nav class="nav-links" role="navigation" aria-label="フリーランスナビゲーション">
                ...
                @endphp
                <a href="{{ route('corporate.applications.index') }}" class="nav-link {{ $totalUnreadCount > 0 ? 'has-badge' : '' }} active">
                    応募した案件
                    @if($totalUnreadCount > 0)
                        <span class="badge" aria-live="polite">{{ $totalUnreadCount }}</span>
                    @endif
                </a>
                <a href="{{ route('corporate.scouts.index') }}" class="nav-link {{ $totalUnreadCount > 0 ? 'has-badge' : '' }}">
                    スカウト
                    @if($totalUnreadCount > 0)
