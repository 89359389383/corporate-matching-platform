    ...
    <header class="header" role="banner">
        <div class="header-content">
            ...
            <nav class="nav-links" role="navigation" aria-label="フリーランスナビゲーション">
                <a href="{{ route('corporate.jobs.index') }}" class="nav-link active">案件一覧</a>
                @php
                    $appUnread = ($unreadApplicationCount ?? 0);
                    $scoutUnread = ($unreadScoutCount ?? 0);
                @endphp
                <a href="{{ route('corporate.applications.index') }}" class="nav-link {{ $appUnread > 0 ? 'has-badge' : '' }}">
                    応募した案件
                    @if($appUnread > 0)
                        <span class="badge" aria-live="polite">{{ $appUnread }}</span>
                    @endif
                </a>
                <a href="{{ route('corporate.scouts.index') }}" class="nav-link {{ $scoutUnread > 0 ? 'has-badge' : '' }}">
                    スカウト
                    @if($scoutUnread > 0)
    ...
    <main class="main-content max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-6 md:py-10 flex flex-col lg:flex-row gap-6 lg:gap-8">
        ...
        <aside class="sidebar w-full lg:w-80 lg:sticky lg:top-[calc(var(--header-height)+1.5rem)]">
            <div class="search-section">
                <h3>検索条件</h3>
                <form method="GET" action="{{ route('corporate.jobs.index') }}" id="searchForm">
                ...
                @forelse($jobs as $job)
                    @php
                        ...
                        if (!is_null($minRate) && !is_null($maxRate) && $minRate !== '' && $maxRate !== '') {
                            $rewardText = number_format((float)$minRate) . '円 〜 ' . number_format((float)$maxRate) . '円';
                        } elseif (!is_null($minRate) && $minRate !== '') {
                            $rewardText = number_format((float)$minRate) . '円〜';
                        } elseif (!is_null($maxRate) && $maxRate !== '') {
                            $rewardText = '〜' . number_format((float)$maxRate) . '円';
                        }
                    ...
                    <article class="card rounded-2xl bg-white border border-slate-200 shadow-sm p-5 md:p-7 relative overflow-hidden">
                        ...
                        <div class="actions flex flex-col md:flex-row md:items-center gap-3 md:gap-4 border-t border-slate-200 pt-4 mt-5">
                            ...
                            @if($isApplied)
                                ...
                                @endphp
                                @if($thread)
                                    <a href="{{ route('corporate.threads.show', $thread['id']) }}" class="btn btn-success w-full md:flex-1">応募済み（チャットを開く）</a>
                                @else
