...
<main class="main">
    <h1 class="title">契約作成（下書き）</h1>
    <p class="sub">スレッドに紐づく契約を作成します（法人には提示するまで通知されません）。</p>
    @include('partials.error-panel')

    <div class="card" style="margin-bottom:1rem;">
        <div style="font-weight:900;">相手: {{ $thread->corporate->display_name ?? '法人' }}</div>
        @if($thread->job)
            <div style="color:#6a737d; font-weight:800; margin-top:0.25rem;">案件: {{ $thread->job->title }}</div>
        @endif
    </div>
