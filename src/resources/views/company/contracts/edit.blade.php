...
<main class="main">
    <h1 class="title">契約編集（draftのみ）</h1>
    <p class="sub">v{{ $contract->version }} / {{ $contract->contract_type }} / 状態: {{ $contract->status }}</p>
    @include('partials.error-panel')

    <form method="POST" action="{{ route('company.contracts.update', ['contract' => $contract]) }}" class="card">
        @csrf
        @method('PATCH')
