...
<main class="main">
    <h1 class="title">新版作成（下書き）</h1>
    <p class="sub">現在の契約（v{{ $base->version }}）をアーカイブし、新しい版を draft として作成します。</p>
    @include('partials.error-panel')

    <form method="POST" action="{{ route('company.contracts.versions.store', ['contract' => $base]) }}" class="card">
        @csrf

        @php
            $terms = $base->terms_json ?? [];
        @endphp

        @include('company.contracts._form_fields', [
            'contractType' => $base->contract_type,
            'startDate' => $base->start_date ? $base->start_date->format('Y-m-d') : '',
            'endDate' => $base->end_date ? $base->end_date->format('Y-m-d') : '',
            'terms' => $terms,
        ])

        <div style="display:flex; gap:0.75rem; margin-top:1.25rem;">
            <a class="btn" href="{{ route('company.contracts.show', ['contract' => $base]) }}">戻る</a>
            <button class="btn primary" type="submit">新版を作成</button>
        </div>
    </form>
</main>
</body>
