@php
    ...
    $isNda = $ct === 'nda';
    $isIndividual = $ct === 'individual';
    $deliverablesInput = old('deliverables', $terms['deliverables'] ?? '');
    $hasDeliverables = trim((string)$deliverablesInput) !== '';
@endphp

<div style="display:grid; gap:0.9rem;">
    <div>
        <label style="font-weight:900; display:block; margin-bottom:0.35rem;">契約タイプ（必須）</label>
        <select name="contract_type" required class="@error('contract_type') is-invalid @enderror" style="width:100%; padding:0.6rem; border:1px solid #e1e4e8; border-radius:10px;">
    ...
    <div style="display:flex; gap:0.75rem;">
        <div style="flex:1;">
            ...
            <input type="date" name="start_date" required value="{{ old('start_date', $startDate ?? '') }}" class="@error('start_date') is-invalid @enderror" style="width:100%; padding:0.6rem; border:1px solid #e1e4e8; border-radius:10px;">
            @error('start_date')
                <span class="error-message">{{ $message }}</span>
            @enderror
        </div>
        <div style="flex:1;">
            <label style="font-weight:900; display:block; margin-bottom:0.35rem;">終了日（{{ $isIndividual ? '必須' : '任意' }}）</label>
    ...
    <div>
        <label style="font-weight:900; display:block; margin-bottom:0.35rem;">契約期間（任意）</label>
    ...
    <div>
        <label style="font-weight:900; display:block; margin-bottom:0.35rem;">取引条件（任意）</label>
    ...
    <div>
        <label style="font-weight:900; display:block; margin-bottom:0.35rem;">金額（{{ $isNda ? '任意' : '必須' }}）</label>
    ...
    <div>
        <label style="font-weight:900; display:block; margin-bottom:0.35rem;">支払条件（{{ $isNda ? '任意' : '必須' }}）</label>
    ...
    <div>
        <label style="font-weight:900; display:block; margin-bottom:0.35rem;">成果物（{{ $isIndividual ? '必須' : '任意' }}）</label>
    ...
    <div>
        <label style="font-weight:900; display:block; margin-bottom:0.35rem;">納期（{{ ($isIndividual || $hasDeliverables) ? '必須' : '任意' }}）</label>
    ...
    <div>
        <label style="font-weight:900; display:block; margin-bottom:0.35rem;">業務範囲（{{ $isNda ? '任意' : '必須' }}）</label>
