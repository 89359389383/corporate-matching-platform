...
<body>
    ...
    <main class="main-content">
        <aside class="sidebar">
            <div class="panel">
                <h3>検索条件</h3>
                <form method="GET" action="{{ route('company.corporates.index') }}">
                    <div class="field">
                    ...
                    <button class="search-btn clear-btn" type="button" onclick="window.location='{{ route('company.corporates.index') }}'">クリア</button>
                </form>
            </div>
        </aside>
        ...
        <section class="content-area flex-1 min-w-0">
            ...
            <div class="list grid grid-cols-1 gap-5" id="corporateList" aria-label="法人一覧">
                @forelse($corporates as $index => $corporate)
                    ...
                    @endphp
                    <article class="card {{ $index === 0 ? 'is-selected' : '' }} rounded-2xl bg-white border border-slate-200 shadow-sm p-5 md:p-6 relative overflow-hidden" tabindex="0" role="button" data-id="{{ $corporate->id }}" aria-pressed="{{ $index === 0 ? 'true' : 'false' }}">
                        <div class="card-header">
                            ...
                            @else
                                <a class="card-scout-btn btn-primary" href="{{ route('company.scouts.create', ['corporate_id' => $corporate->id]) }}" aria-label="スカウト">スカウト</a>
                            @endif
                        </div>
                        <div class="row">
                            <div class="avatar" aria-hidden="true">{{ $avatarText }}</div>
                            <div style="min-width:0; flex: 1; overflow: hidden;">
                                <div class="name">{{ $corporate->display_name }}</div>
                                <div class="sub">{{ $corporate->job_title ?? '' }}</div>
                ...
                <div class="panel" id="detailPanel" aria-live="polite">
                    ...
                    <div id="detailContent" style="display: none;">
                        ...
                        <div class="detail-actions">
                            ...
                            @else
                                <a class="btn btn-primary" id="dScoutLink" href="{{ $firstCorporate ? route('company.scouts.create') . '?corporate_id=' . $firstCorporate->id : '#' }}">スカウト</a>
                            @endif
                        </div>
                    </div>
        ...
        (function () {
            ...
            const render = (id) => {
                ...
                if (x.threadId) {
                    ...
                    dScoutLink.href = '{{ route("company.scouts.create") }}?corporate_id=' + id;
